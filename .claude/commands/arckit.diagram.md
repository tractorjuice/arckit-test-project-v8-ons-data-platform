...
8. **IMPORTANT - Mermaid Syntax**: C4 diagrams support pipe separators for multi-line content (e.g., `Container(api, "API", "Node.js | Express", "Description")`), but flowcharts do NOT support pipes in node labels. For flowcharts, use HTML line breaks instead (e.g., `Node["Line 1<br/>Line 2<br/>Line 3"]`). Never use pipe characters in flowchart node labels or edge labels as they cause parsing errors.
